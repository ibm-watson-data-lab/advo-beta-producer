...
View your app at: http://localhost:9080/LocalCartKafkaProducer
...
The manifest.yml is provided in the sample.
  ...
  ```
  applications:
  - path: target/LocalCartKafkaProducer.war
    memory: 512M
    instances: 1
...
4. Run your application locally on Liberty:
  - Right click on the `LocalCartKafkaProducer` sample and select `Run As` -> `Run on Server` option
  - Find and select the localhost Liberty server and press `Finish`
  - In a few seconds, your application should be running at http://localhost:9080/LocalCartKafkaProducer/
...
4. Your Liberty server in Eclipse should automatically pick up the changes and restart the application.
  ...
  View your app at: http://localhost:9080/LocalCartKafkaProducer/. Any names you enter into the app will now get added to the database.
